...
const STOP_WORDS = /\b(add|create|new|schedule|edit|change|update|move|delete|remove|list|show|view|see|task|for|to|on|my|a)\b/g;
...
function extractTitle(text, intent) {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(text.trim()) || // ISO
